...
class RotateHandler extends TransformHandler {
    constructor(camera) {
        ...
        this.onMouseMove = (mouseEvent)=> {
            if(!this.object) {
                oldMX = mouseEvent.clientX;
                oldMY = mouseEvent.clientY;
                return false;
            }
            //
            const clientX = mouseEvent.clientX;
            ...
            if(!mouseEvent.buttons || mouseEvent.button !== THREE.MOUSE.LEFT) {
                ...
                let deltaX = mouseEvent.clientX - oldMX;
                let deltaY = oldMY - mouseEvent.clientY;
                //
...
class ModelTransformTool extends ToolBase {
    constructor(viewer, camera) {
        ...
        this.size = 1;
        //
        let translateHandler = new TranslateHandler(camera);
        this.tempObj.add(translateHandler);
        //
        let rotateHandler = new RotateHandler(camera);
        this.tempObj.add(rotateHandler);
        //
        let scaleHandler = new ScaleHandler(camera);
